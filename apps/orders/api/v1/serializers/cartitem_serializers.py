    ...
    def validate_menu_item(self, value):
        ...
        try:
            ...
            existing = CartItem.objects.filter(
                cart=request.user.customer_profile.cart, menu_item=value
            ).first()
            if existing:
                raise serializers.ValidationError("Item already exists in your cart.")
        except Cart.DoesNotExist:
            return value

        if cart.restaurant and value.restaurant != cart.restaurant:
            raise serializers.ValidationError(
                "Clear your cart first to add another restaurant's item."
            )
        return value
