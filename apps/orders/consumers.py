        ...
        try:
            order = (
                Order.objects.select_related(
                    "customer__user", "restaurant__owner", "driver__user"
                )
...
class RestaurantDashboardConsumer(AsyncWebsocketConsumer):
    ...
    def user_owns_restaurant(self, user, restaurant_id):
        ...
        try:
            return Restaurant.objects.filter(
                id=restaurant_id, owner=user, is_deleted=False
            ).exists()
        except Exception:
            return False
    ...
    async def connect(self):
        ...
        has_access = await self.user_owns_restaurant(
            user=user, restaurant_id=self.restaurant_id
        )
        if not has_access:
            await self.close()
            return
    ...
    def user_validation(self, user, customer_id):
        ...
        try:
            return CustomerProfile.objects.filter(
                id=customer_id, user=user, is_deleted=False
            ).exists()
        except Exception:
            return False
    ...
    def user_validation(self, user, driver_id):
        ...
        try:
            return DriverProfile.objects.filter(
                id=driver_id, user=user, is_deleted=False
            ).exists()
        except Exception:
            return False
