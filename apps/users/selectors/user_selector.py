from apps.users.models import CustomerProfile, DriverProfile, Address
...
class UserSelector:
    ...
    def get_available_driver():
        ...
        return (
            ...
            .filter(is_available=True, is_deleted=False)
            .first()
        )
    ...
    def get_address_queryset(*, user):
        ...
        return Address.objects.select_related("user").filter(
            user=user, is_deleted=False
        )
    ...
    def get_address(*, address_id, user):
        ...
        return Address.objects.filter(
            id=address_id, user=user, is_deleted=False
        ).first()
