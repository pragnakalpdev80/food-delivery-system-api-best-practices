...
class CustomerAPITestCase(APITestCase):
    ...
    def test_retrieve_user(self):
        ...
        response = self.client.get(
            f"/api/v1/customers/{self.user.customer_profile.id}/"
        )  # GET specific article
        self.assertEqual(response.status_code, status.HTTP_200_OK)  # Verify success
        self.assertEqual(
            response.data["id"], self.user.customer_profile.id
        )  # Check correct article returned

    def test_retrieve_another_user(self):
        """Test retrieving single user using another user's JWT"""
        response = self.client.get("/api/v1/customers/1/")
        self.assertEqual(
...
class DriverAPITestCase(APITestCase):
    ...
    def test_retrieve_another_user(self):
        """Test retrieving single user using another user's JWT"""
        response = self.client.get("/api/v1/drivers/1/")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_update_driver(self):
        update_data = {"vehicle_type": "car"}
        response = self.client.patch(
            f"/api/v1/drivers/{self.user.driver_profile.id}/", update_data
        )  # GET specific article
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.user.refresh_from_db()
        self.assertEqual(
            response.data["vehicle_type"], self.user.driver_profile.vehicle_type
        )  # Check correct article returned
...
class RestaurantAPITestcase(APITestCase):
    ...
    def test_update_restaurant_success(self):
        ...
        response = self.client.patch(
            f"/api/v1/restaurants/{self.restaurant.id}/", update_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class MenuAPITestcase(APITestCase):
    ...
    def test_create_menu(self):
        ...
        response = self.client.post("/api/v1/menu-items/", self.valid_data)
    ...
    def test_update_menu_fail(self):
        ...
        response = self.customer_client.patch(
            f"/api/v1/menu-items/{self.restaurant.id}/", update_data
        )
...
class CartAPITestcase(APITestCase):
    def setUp(self):
        ...
        self.rest_client.force_authenticate(
            self.restaurant_owner, restaurant_owner_token
        )
        self.rest_client.credentials(
            HTTP_AUTHORIZATION=f"Bearer {restaurant_owner_token}"
        )
        customer_token = AccessToken.for_user(user=self.customer)
        self.customer_client.force_authenticate(self.customer, customer_token)
        self.customer_client.credentials(HTTP_AUTHORIZATION=f"Bearer {customer_token}")
    ...
    def test_retrieve_cart_success(self):
        ...
        response = self.customer_client.get(
            f"/api/v1/cart/{self.customer.customer_profile.cart.id}/"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_add_item_cart_success(self):
        """Test retrieving single cart"""
        response = self.customer_client.post("/api/v1/cart-items/", self.valid_data)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class OrderAPITestcase(APITestCase):
    def setUp(self):
        ...
        self.address = Address.objects.create(
            user=self.customer, address_name="string", address="string", is_default=True
        )
        self.driver = User.objects.create_user(
            email="testdriver@gmail.com",
            password="testpass123",
        ...
        self.valid_data = {"status": "confirmed"}

        self.invalid_data = {"status": "ready"}

        self.driver_valid_data = {"status": "picked_up"}

        self.driver_invalid_data = {"status": "confirmed"}
    ...
    def test_update_order_restaurant_success(self):
        response = self.rest_client.post(
            f"/api/v1/orders/{self.order.id}/update-status/", self.valid_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_update_order_restaurant_fail(self):
        response = self.rest_client.post(
            f"/api/v1/orders/{self.order.id}/update-status/", self.invalid_data
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_update_order_driver_success(self):
        response = self.driver_client.post(
            f"/api/v1/orders/{self.order2.id}/update-status/", self.driver_valid_data
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_update_order_driver_fail(self):
        response = self.driver_client.post(
            f"/api/v1/orders/{self.order2.id}/update-status/", self.driver_invalid_data
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
