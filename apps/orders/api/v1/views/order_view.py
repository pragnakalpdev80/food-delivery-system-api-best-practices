...
class OrderViewSet(viewsets.ModelViewSet):
    ...
    def update_status(self, request, *args, **kwargs):
        ...
        if request.user.user_type == "customer":
            return Response(
                {"message": "Only Restaurants and Driver can update order status."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        OrderService.update_status(
            order=order, user_type=user_type, new_status=new_status
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {
                "error": "You cannot create orders directly use place method to create orders"
            },
